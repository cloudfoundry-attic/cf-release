...
public class Graphs {
  ...
  private static String[] COMMON_ERRORS = {"400", "401", "402", "403", "404", "500", "501", "502"};
  ...
  private void registerGraphs() {
    ...
    registerBillingGraph("billing.requests", "Requests");
    registerBillingGraph("billing.success", "Successful");
    registerBillingGraph("billing.cpu", "CPU");
    
    for (String error: COMMON_ERRORS) {
      registerBillingErrorGraph(error);
    }
  ...
  private void registerBillingGraph(final String id, final String label) {
    Provider<GraphPresenter> provider = new Provider<GraphPresenter>() {
  ...
  private void registerBillingErrorGraph(final String error) {
    final String id = "billing.errors." + error;
    Provider<GraphPresenter> provider = new Provider<GraphPresenter>() {

      @Override
      public GraphPresenter get() {
        return graphProvider.get()
            .setTitle(error + " errors")
            .setGraphId(id)
            .addQuery(new Query.Builder()
                .setAggregator(Query.Aggregator.SUM)
                .setDownsamplingAggregator(Query.Aggregator.AVG)
