...
export ENABLE_RABBITMQ="yes"
if [ -f /etc/default/rabbitmq ]; then . /etc/default/rabbitmq; fi
if [ "x$ENABLE_RABBITMQ" = "xyes" ]; then exec start-stop-daemon --start --quiet --chuid vcap --exec /var/vcap/packages/rabbitmq/sbin/rabbitmq-server >/store/log/rabbitmq_stdout.log 2>/store/log/rabbitmq_stderr.log; fi
